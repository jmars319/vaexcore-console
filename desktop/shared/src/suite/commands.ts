...
export const validateSuiteCommandDocument = (
  document: SuiteCommandDocument,
) => {
  if (document.schemaVersion !== SUITE_DISCOVERY_SCHEMA_VERSION) {
    throw new Error(
      `Suite command schemaVersion must be ${SUITE_DISCOVERY_SCHEMA_VERSION}.`,
    );
  }
  if (!document.sourceApp || !suiteAppIds.has(document.sourceApp)) {
    throw new Error(
      `Suite command sourceApp is unknown: ${document.sourceApp ?? "(missing)"}.`,
    );
  }
